...
class Welcome extends AdminBaseController {
	...
	public function saveupdateeuser() {
        ...
        $this->form_validation->set_rules('password','Password','required|min_length[80]|alpha_numeric');
		// $this->form_validation->set_rules('cpassword', 'Confirm Password', 'required|matches[password]|min_length[80]');
		if ($this->form_validation->run() == false){	
			$this->session->set_flashdata('erroruserlog', '<div class="common_error_div">'.validation_errors().' </div> ');
